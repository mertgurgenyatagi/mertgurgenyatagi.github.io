...
function MiniCountdownDigit({ value, label }: { value: number; label: string }) {
  return (
    <span className="flex items-baseline gap-1 whitespace-nowrap">
      <span className="font-display text-2xl leading-none font-semibold text-color_text tabular inline-block w-[2ch] text-center sm:text-3xl">
      ...
      <FrameBody className="flex flex-col gap-4 px-4 py-3.5 sm:flex-row sm:items-center sm:justify-between sm:px-5">
        ...
        <div className="flex flex-wrap items-center gap-5 sm:gap-6">
          {showCta && (
            <Link
              to="/predictions"
              className="inline-flex shrink-0 items-center gap-2 rounded-full bg-accent px-5 py-2.5 font-display text-xs font-semibold uppercase tracking-wider text-main outline-none transition-all duration-150 ease-[var(--ease-cotton)] hover:bg-accent/85 hover:-translate-y-0.5 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-color_accent"
            >
